function [hitsA,countsA,hits2,counts2,results]=summarizeTwitter(texts,times)
    ...
    results=cell(n,1);
    parfor i = 1:n
        [hitsAN(:,:,i),countsAN(:,:,i),hits2N(:,:,i),counts2N(:,:,i),results{i}]=summarizeResults1(texts{i},times{i});
    end
...
function [hitsA,countsA,hits2,counts2,results]=summarizeResults1(texts,times)
    results = prepareAll(texts,times);
...
function result = prepareAll(target,times)
    n=size(target,1);
    if n>1000
        sets=cell(n-1000,1);
        for i = 1000+1:n
            base=target(i-1:-1:i-1000,:);
            subtimes=times(i)-times(i-1:-1:i-1000);
            items=base(base~=0);
            items=unique(items);
            m=length(items);
            hold=zeros(m,4);
