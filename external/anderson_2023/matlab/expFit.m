function [stats,lagsP,hitsAP,countsAP,hits2P,counts2P] = expFit(params,lags,n,m)
    load('base32.mat', 'bounds')
    load('base32.mat', 'bounds5')
    v=params(1);
    b=params(2);
    probs = schoolerTemplates(params(3),1/params(4),n);
    desirabilities = random('gamma',v,b,m,n);
    hitsAP=zeros(1000,225,m);
    countsAP=zeros(1000,225,m);
    hits2P=zeros(1000,1000,m);
    counts2P=zeros(1000,1000,m);
    parfor i = 1:m
        [hitsAP(:,:,i),countsAP(:,:,i),hits2P(:,:,i),counts2P(:,:,i)]=summarize(probs,desirabilities(i,:));
    end
    hitsAP=sum(hitsAP,3);
...
function probs =schoolerTemplates(alpha,beta,n)
    decays=random('exp',alpha,1,n);
    p=exp(-beta)*beta;
    revivals=rand(3000,n)<p;
    probs=zeros(3000,n);
    starts=round(random('exp',1/beta,1,n));
    for i = 1:n
        times=zeros(3000,1);
        index=starts(i);
        resets=[0;find(revivals(:,i)==1);3000];
        lens=resets(2:end)-resets(1:end-1);
        for j = 1:length(lens)
                times(resets(j)+1:resets(j+1))=index+[1:lens(j)];
                index=0;
        end
        probs(:,i)=exp(-decays(i)*times);
    end
end

function [hitsAP,countsAP,hits2P,counts2P]=summarize(probs,desirabilities) 
    probs=probs.*desirabilities;
    history=rand(size(probs))<probs;
